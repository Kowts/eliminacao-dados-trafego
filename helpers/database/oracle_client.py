...
class OracleClient(BaseDatabase):
    ...
    def initialize_oracle_client(self):
        ...
        if self.client_initialized:
            logging.info("Oracle Client is already initialized.")
            return

        try:
            # Determine OS and set the Oracle Client library directory
            if sys.platform.startswith("win"):
                lib_dir = os.getenv("ORACLE_CLIENT_PATH", r"C:\instantclient_11_2")
            else:
                lib_dir = os.getenv("ORACLE_CLIENT_PATH", "/opt/oracle/instantclient")
